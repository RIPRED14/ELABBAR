...
const Stockage = {
      ...
      <div class="fade-in">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:22px;">
          <div>
            ...
            <p class="page-subtitle">Gestion des flux de marchandise</p>
          </div>
          <div style="display:flex;gap:8px;">
            <button class="btn btn-outline" onclick="Stockage.printList()" title="Imprimer">🖨️ Imprimer</button>
            ${this.currentTab === 'entrees' ? `<button class="btn btn-success" onclick="Stockage.startScanner()" title="Scanner QR">📷 Scanner</button><button class="btn btn-primary" onclick="Stockage.showForm()">+ Nouvelle Entrée</button>` : ''}
            ${this.currentTab === 'sorties' ? `<button class="btn btn-primary" onclick="Stockage.showSortieForm()">+ Nouvelle Sortie</button>` : ''}
          </div>
        </div>

        <div class="tabs" style="margin-bottom:18px;">
          <div class="tab ${this.currentTab==='entrees'?'active':''}" onclick="Stockage.switchTab('entrees')">📥 Entrées (Réception)</div>
          <div class="tab ${this.currentTab==='sorties'?'active':''}" onclick="Stockage.switchTab('sorties')">📤 Sorties Stock</div>
          <div class="tab ${this.currentTab==='pending'?'active':''}" onclick="Stockage.switchTab('pending')">⏳ Éléments en Attente${pendingCount > 0 ? ` <span style="background:var(--accent-red,#ef4444);color:#fff;border-radius:50%;padding:2px 7px;font-size:0.72rem;font-weight:800;margin-left:4px;">${pendingCount}</span>` : ''}</div>
          <div class="tab ${this.currentTab==='mouvements'?'active':''}" onclick="Stockage.switchTab('mouvements')">🔄 Mouvements & Traçabilité</div>
        </div>
  ...
  renderEntrees() {
    ...
    return `
      <div class="kpi-grid">
        <div class="kpi-card purple"><div class="kpi-icon purple">📥</div><div class="kpi-label">Entrées ce mois</div><div class="kpi-value">${this.getMonthEntries().length}</div></div>
        <div class="kpi-card blue"><div class="kpi-icon blue">📦</div><div class="kpi-label">Total Caisses</div><div class="kpi-value">${App.formatNumber(this.getMonthEntries().reduce((s,e)=>s+e.lignes.reduce((ss,l)=>ss+(l.nbCaisses||0),0),0),0)}</div></div>
        <div class="kpi-card green"><div class="kpi-icon green">⚖️</div><div class="kpi-label">Poids Brut Total</div><div class="kpi-value">${App.formatNumber(this.getMonthEntries().reduce((s,e)=>s+e.lignes.reduce((ss,l)=>ss+(l.pdsBrutTotal||0),0),0),0)}<span class="kpi-unit">kg</span></div></div>
        <div class="kpi-card cyan"><div class="kpi-icon cyan">🚢</div><div class="kpi-label">Bateaux distincts</div><div class="kpi-value">${new Set(this.getMonthEntries().flatMap(e=>[e.bateau, ...e.lignes.map(l=>l.bateau)]).filter(Boolean)).size}</div></div>
      </div>

      <div class="card">
        <div class="card-header">
          <span class="card-title">📋 Historique des entrées (${entries.length})</span>
        </div>
        <div class="card-body"><div class="table-container" id="stockageListTable">${this.buildListTable()}</div></div>
      </div>
    `;
  },
    ...
    return `<table>
      ...
      <tbody>${entries.map(e => {
        ...
        return `<tr>
          ...
          <td>${e.client}</td>
          <td>${e.fournisseur}</td>
          <td><span class="badge badge-info">${e.origine}</span></td>
          ...
          <td class="td-center">
            <button class="btn-icon" onclick="Stockage.viewEntry(${e.id})" title="Voir détail"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
            <button class="btn-icon" onclick="Stockage.editEntry(${e.id})" title="Modifier"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg></button>
    ...
    container.innerHTML = `
      <div class="card slide-up" style="margin-bottom:22px;">
        <div class="card-header" style="background:var(--gradient-purple);border-radius:var(--radius-md) var(--radius-md) 0 0;">
          <span class="card-title" style="color:white;">${entry ? '✏️ Modifier l\'entrée' : '📥 Nouvelle Entrée Stockage'}</span>
          <button class="btn-icon" style="border-color:rgba(255,255,255,0.3);color:white" onclick="Stockage.hideForm()"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg></button>
        </div>
        <div class="card-body">
          ...
          <div style="display:flex;gap:15px;justify-content:center;margin-top:30px;padding:25px 0; border-top: 1px solid var(--border-color);">
            <button class="btn btn-outline" style="min-width: 150px;" onclick="Stockage.hideForm()">✕ Annuler</button>
  ...
  async processOCR(event) {
    ...
    const loadingArea = document.getElementById('ocrLoadingAreaStock');
...
Renvoie un objet JSON avec la structure exacte suivante :
{
  ...
  "lignes": [
    { "espece": "Nom espece", "calibre": "Taille", "nbCaisses": 0, "poidsReçu": 0.0, "emballage": "Plastique/Carton" }
  ]
}
Notes:
...
- Les nombres doivent être au format décimal avec un point.`;
      ...
      if (data.lignes && data.lignes.length > 0) {
        ...
        data.lignes.forEach(l => {
          const entryLine = this.emptyLigne();
          entryLine.espece = (l.espece || '').toUpperCase();
          entryLine.calibre = (l.calibre || '').toUpperCase();
          entryLine.nbCaisses = parseFloat(l.nbCaisses) || 0;
          entryLine.pdsNetTotal = parseFloat(l.poidsReçu) || 0;
          entryLine.pdsBL = entryLine.pdsNetTotal;
          entryLine.emballage = l.emballage || 'Plastique';
          
          const tr = document.createElement('tr');
          const idx = tbody.children.length;
          tr.innerHTML = this.renderLigneRow(entryLine, idx);
          tbody.appendChild(tr);
        });
        
        this.calcTotal();
        App.toast(`IA : ${data.lignes.length} lignes extraites !`, 'success');
      }
    } catch (err) {
      console.error(err);
  ...
  viewEntry(id) {
    ...
    App.showModal(`📥 Entrée ${e.reference}`, `
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:18px;">
        ...
        <div><span style="color:var(--text-muted);font-size:0.8rem;">Origine</span><br><span class="badge badge-info">${e.origine}</span></div>
      </div>
  ...
  showSortieForm() {
    ...
    container.innerHTML = `
      <div class="card slide-up" style="margin-bottom:22px;">
        <div class="card-header" style="background:var(--gradient-blue);border-radius:var(--radius-md) var(--radius-md) 0 0;">
          <span class="card-title" style="color:white;">📤 Nouvelle Sortie Stockage</span>
          <button class="btn-icon" style="border-color:rgba(255,255,255,0.3);color:white" onclick="Stockage.hideForm()"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg></button>
        </div>
        <div class="card-body">
  ...
  renderMouvements() {
    ...
    return `
      <div class="card" style="margin-bottom:22px;">
        <div class="card-header" style="background:var(--gradient-purple);border-radius:var(--radius-md) var(--radius-md) 0 0;">
          <span class="card-title" style="color:white;">🔍 Recherche Avancée (Traçabilité)</span>
        </div>
        <div class="card-body">
  ...
  traceLot(recId, lineIdx) {
    ...
    container.innerHTML = `
      <div class="card slide-up">
        <div class="card-header" style="background:var(--gradient-purple);border-radius:var(--radius-md) var(--radius-md) 0 0;">
          <span class="card-title" style="color:white;">🔄 Parcours du Lot ${entry.reference} — Palette: ${line.palette || '-'}</span>
        </div>
        <div class="card-body">
  ...
  renderPendingEntries() {
    ...
    const validated = (App.data.pendingStorageEntries || []).filter(e => e.status === 'validated');
    const chambreLabels = { chambre1: 'Chambre 1', chambre2: 'Chambre 2', entreposage: 'Entreposage' };

    return `
      <div class="kpi-grid">
        <div class="kpi-card orange"><div class="kpi-icon orange">⏳</div><div class="kpi-label">En attente</div><div class="kpi-value">${pending.length}</div></div>
        <div class="kpi-card green"><div class="kpi-icon green">✅</div><div class="kpi-label">Validés</div><div class="kpi-value">${validated.length}</div></div>
        <div class="kpi-card blue"><div class="kpi-icon blue">⚖️</div><div class="kpi-label">Poids PF total (en attente)</div><div class="kpi-value">${App.formatNumber(pending.reduce((s,e) => s + (e.poidsPF||0), 0), 0)}<span class="kpi-unit">kg</span></div></div>
        <div class="kpi-card purple"><div class="kpi-icon purple">📦</div><div class="kpi-label">Caisses PF (en attente)</div><div class="kpi-value">${App.formatNumber(pending.reduce((s,e) => s + (e.caissesPF||0), 0), 0)}</div></div>
      </div>

      <div class="card">
        <div class="card-header">
          <span class="card-title">⏳ Historique des éléments en attente (${pending.length} en attente / ${validated.length} validés)</span>
        </div>
        <div class="card-body">
          <div class="table-container">
            ${(App.data.pendingStorageEntries || []).length === 0
              ? '<div class="empty-state"><div class="empty-state-icon">📭</div><div class="empty-state-text">Aucun élément en attente</div><div style="color:var(--text-muted);font-size:0.85rem;margin-top:8px;">Les éléments apparaîtront ici quand un traitement ou reconditionnement sera envoyé vers le stockage.</div></div>'
              : `<table>
                <thead><tr>
                  <th>Date Envoi</th><th>Origine</th><th>Espèce</th><th>Calibre</th><th>Client</th>
                  <th class="td-right">Poids PF</th><th class="td-right">Caisses</th><th>Chambre</th><th>Statut</th><th>Actions</th>
                </tr></thead>
                <tbody>${[...pending, ...validated].sort((a,b) => new Date(b.dateEnvoi) - new Date(a.dateEnvoi)).map(e => {
                  ...
                  return `<tr style="${!isPending ? 'opacity:0.6;' : ''}">
                    ...
                    <td class="td-center" style="white-space:nowrap;">
                      <button class="btn-icon" onclick="Stockage.viewPendingEntry(${e.id})" title="Voir (lecture seule)"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></button>
  ...
  viewPendingEntry(id) {
    ...
    const qrData = JSON.stringify({
      type: 'pending_storage', id: e.id, productionId: e.productionId,
      origine: e.origine, espece: e.espece, calibre: e.calibre,
      client: e.client, poidsPF: e.poidsPF, chambre: e.chambreDestination,
      date: e.dateEnvoi
    });

    App.showModal(`🔒 Fiche en Lecture Seule — ${e.origine}`, `
      <div style="background:rgba(245,158,11,0.1);border:1px solid rgba(245,158,11,0.3);border-radius:8px;padding:10px 14px;margin-bottom:18px;display:flex;align-items:center;gap:10px;">
        <span style="font-size:1.3rem;">🔒</span>
        <span style="font-size:0.85rem;color:var(--accent-yellow);font-weight:600;">Cette fiche est en lecture seule et ne peut pas être modifiée.</span>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:18px;">
        ...
        <div><span style="color:var(--text-muted);font-size:0.8rem;">Rendement</span><br><strong>${App.formatNumber(e.rendement||0,2)}%</strong></div>
        <div><span style="color:var(--text-muted);font-size:0.8rem;">Poids MP</span><br><strong>${App.formatNumber(e.poidsMP||0,2)} kg</strong></div>
        <div><span style="color:var(--text-muted);font-size:0.8rem;">Prix de revient</span><br><strong>${App.formatNumber(e.prixRevient||0,2)} DH/kg</strong></div>
      </div>
